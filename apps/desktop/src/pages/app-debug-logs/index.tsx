...
export default function LogPage(): React.JSX.Element {
    ...
    onSuccess: (result) => {
      if (result.success) {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: result.error || "Failed to clear log file",
        });
      }
  ...
  const isLogEntryStart = (line: string): boolean => {
    // Check for timestamp pattern like [2025-11-28 10:17:13.583] or [2025-11-28T10:17:13.583Z]
    return /^\[\d{4}-\d{2}-\d{2}[\sT]\d{2}:\d{2}:\d{2}/.test(line.trim());
  };
          ...
          <div className="flex items-center gap-2">
            ...
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button variant="outline" disabled={isClearingLogs || isLoading} className="gap-2">
